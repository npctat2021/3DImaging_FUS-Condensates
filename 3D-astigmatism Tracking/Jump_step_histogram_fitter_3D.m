function Jump_step_histogram_fitter_3D
 
lb = [0.00, 0.0, 0.5, 0, 0];  %lower bound
ub = [0.02, 0.5, 1.0, 1, 1];  %upper bound
A = [0 0 0 1 1]; % Indicates that A1+A2 <= 1 and speeds up the fitting routine. 
B = [1];
 
[x,fval] = ga(@Diff_coeff_3, 5, A, B,[],[],lb, ub);

%% Global Optimization fitting routine
function f = Diff_coeff_3 (x)
...
exp_data = readmatrix("Experiment.xlsx");
...
for j= 1:1:loc_number
    ...
    elseif rand_nm(j,1) <= (a_1 + a_2) && rand_nm(j,1) > a_1
        ...
        sigma_df = (2*(10^6)*dc_3*(time/1000))^0.5; %Adding jumps from diff coeff
        x_loc2(j,1)= x_loc1(j,1) + norminv(rand(),0,sigma_df);
        y_loc2(j,1)= y_loc1(j,1) + norminv(rand(),0,sigma_df);
        z_loc2(j,1)= z_loc1(j,1) + norminv(rand(),0,sigma_df);
...
save("Jump_step_histogram_fitter_3D.mat","x",'-mat');
end