function Jump_step_histogram_fitter_1D

lb = 0; %lower bound 
ub = 1; %upper bound

[x,fval] = ga(@Diff_coeff_1, 1, [],[],[],[], lb, ub);

%% Global Optimization fitting routine
function f = Diff_coeff_1(x)
...
for j= 1:1:loc_number
    ...
    sigma_df = (2*(10^6)*dc*(time/1000))^0.5; %Adding jumps from diff coeff
...
save("Jump_step_histogram_fitter_1D.mat","x",'-mat');
end
